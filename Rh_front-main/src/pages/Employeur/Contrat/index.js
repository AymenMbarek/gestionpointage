...
const Contrat = props =>  {
  ...
  useEffect(() => {
    ...
    console.log("props contrats",props)
  props.getContrats()
    }, [])
              ...
              <Card>
                ...
                {/* start table list */}
                  <div className="table-responsive">
                    <MaterialTable icons={tableIcons} title="Contrats" data={props.contrats["hydra:member"]} columns={columns}
                    options={{columnsButton:true,actionsColumnIndex: -1, exportButton: true}}  
                     actions={[{
                      icon: () => <Edit />,
                      tooltip: 'Edit Contrat',
                      iconProps: { color: "primary" },
                      onClick: (event, rowData) => props.history.replace("/" + 'edit_contrat/'+ rowData.id)
